...
void UpdateVisionNetworkTable(double avgX, double avgY, double avgWidth, double avgHeight, 
	...
	outputEntries[6].SetDouble(watchdog++);		// heightDiff);
	outputEntries[7].SetDouble(heightDiff);		// xOffsetPx);
	outputEntries[8].SetDouble(xOffsetPx); 		// watchdog++);   fixed order of these values..
	nt->Flush();
}
...
void UpdateVisionFoundNetworkTable( bool found ) {
	...
	outputEntries[6].SetDouble(watchdog);
	nt->Flush();
}
...
void GripPipeline::Process(cv::Mat& source0){
	...
	// --debug-- std::string suffix = std::to_string(watchdog) + ".png";
	

	std::vector<cv::RotatedRect> rotatedRectangles;

	// NEW CODE
	// --------if less than 2 things, then just return....
	...
	try {
		...
		/* --debug-- if (watchdog % 100*20 == 0) {
  		...
  		wpi::errs() << "GripPipeline error in sort section.  Error code: '" << errCode << "' \n";
		watchdog++;
		UpdateVisionFoundNetworkTable( false );
	}
	...
	try {
		...
		// --debug-- std::cout << "Highest: " << highest.center.y << std::endl;
		// --debug-- std::cout << "Lowest: " << lowest.center.y << std::endl;
		...
		//                       avg X, avg Y, avg Width, avg Height, stripe count, 
		//						height diff, x offset,
		//						y offset
		...
		UpdateVisionFoundNetworkTable( true );
  		...
  		wpi::errs() << "GripPipeline error in processing section.  Error code: '" << errCode << "' \n";
		watchdog++;
		UpdateVisionFoundNetworkTable( false );
	}
}
